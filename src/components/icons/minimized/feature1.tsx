// src/components/icons/minimized/feature1.tsx

export default function FeatureIcon1() {
  return (
    <svg
      width="376"
      height="317"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      // xmlns:xlink="http://www.w3.org/1999/xlink"
    >
      <rect x="31" y="37" width="314" height="249" rx="16" fill="#F9F9F9" />
      <rect
        x="31"
        y="37"
        width="314"
        height="249"
        rx="16"
        fill="url(#prefix__pattern0)"
        fill-opacity=".53"
      />
      <rect
        x="31"
        y="37"
        width="314"
        height="249"
        rx="16"
        fill="url(#prefix__paint0_linear_725_551)"
        fill-opacity=".38"
      />
      <rect
        x="120.5"
        y="110.5"
        width="161"
        height="92.226"
        rx="8.67"
        fill="#fff"
        stroke="#A8AEB1"
      />
      <rect
        x="128"
        y="117"
        width="5.552"
        height="5.552"
        rx="2.776"
        fill="#E0467E"
      />
      <rect
        x="136.724"
        y="117"
        width="5.552"
        height="5.552"
        rx="2.776"
        fill="#E6D540"
      />
      <rect
        x="145.448"
        y="117"
        width="5.552"
        height="5.552"
        rx="2.776"
        fill="#AAE3C4"
      />
      <rect x="132" y="134" width="60" height="54" rx="8" fill="#69757B" />
      <rect x="202" y="140" width="24" height="6" rx="2" fill="#48D0A8" />
      <rect x="202" y="154" width="24" height="6" rx="2" fill="#A8AEB1" />
      <rect x="243" y="170" width="24" height="6" rx="2" fill="#A8AEB1" />
      <rect
        x="214.5"
        y="223.5"
        width="161"
        height="92.226"
        rx="8.67"
        fill="#fff"
        stroke="#A8AEB1"
      />
      <rect
        x="222"
        y="230"
        width="5.552"
        height="5.552"
        rx="2.776"
        fill="#E0467E"
      />
      <rect
        x="230.724"
        y="230"
        width="5.552"
        height="5.552"
        rx="2.776"
        fill="#E6D540"
      />
      <rect
        x="239.448"
        y="230"
        width="5.552"
        height="5.552"
        rx="2.776"
        fill="#AAE3C4"
      />
      <rect x="226" y="247" width="31" height="21" rx="2" fill="#DDE0E3" />
      <rect x="226" y="276" width="31" height="21" rx="2" fill="#DDE0E3" />
      <rect x="328" y="247" width="24" height="6" rx="2" fill="#48D0A8" />
      <rect x="270" y="247" width="24" height="6" rx="2" fill="#A8AEB1" />
      <rect x="311" y="283" width="24" height="6" rx="2" fill="#A8AEB1" />
      <rect
        x=".5"
        y=".5"
        width="161"
        height="92.226"
        rx="8.67"
        fill="#fff"
        stroke="#A8AEB1"
      />
      <rect
        x="8"
        y="7"
        width="5.552"
        height="5.552"
        rx="2.776"
        fill="#E0467E"
      />
      <rect
        x="16.724"
        y="7"
        width="5.552"
        height="5.552"
        rx="2.776"
        fill="#E6D540"
      />
      <rect
        x="25.448"
        y="7"
        width="5.552"
        height="5.552"
        rx="2.776"
        fill="#AAE3C4"
      />
      <rect x="37" y="24" width="88" height="31" rx="8" fill="#DDE0E3" />
      <rect x="67" y="68" width="24" height="6" rx="2" fill="#48D0A8" />
      <rect x="18" y="68" width="24" height="6" rx="2" fill="#A8AEB1" />
      <rect x="116" y="78" width="24" height="6" rx="2" fill="#A8AEB1" />
      <defs>
        <linearGradient
          id="prefix__paint0_linear_725_551"
          x1="49.5"
          y1="9"
          x2="284.5"
          y2="257"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#0021CD" />
          <stop offset="1" stop-color="#8CFFFF" />
        </linearGradient>
        <pattern
          id="prefix__pattern0"
          patternContentUnits="objectBoundingBox"
          width=".204"
          height=".257"
        >
          <use
            // xlink:href="#prefix__image0_725_551"
            transform="scale(.0016 .002)"
          />
        </pattern>
        <image
          id="prefix__image0_725_551"
          width="128"
          height="128"
          // xlink:href="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
}
